...
final class CatalogPublicationBuilderNonServiceSignals
{
    ...
    private function matchesHiringOrStaffing(string $lower): bool
    {
        $patterns = [
            ...
            // «Требуется … строитель» (напр. «помощник - строитель»); (?<!\p{L}) — не срабатывать на «не требуется»
            '/(?<!\p{L})требуется.{0,120}строител/u',
            // типичный найм: оплата из расчёта за час
            ...
            // «бригада» и «10-15 человек» в одном фрагменте (часто с переносами строк)
            '/\bбригада\b.{0,80}\d+\s*[-–—]\s*\d+\s*человек/u',
            '/\bкто\s+свободен/u',
            '/\bесть\s+кто\s+(?:на\s+)?(?:объект|смену)/u',
            '/\bаванс\s+на\s+\d/u',
            // набор маляров / гипсокартонщиков формулировкой заказчика
