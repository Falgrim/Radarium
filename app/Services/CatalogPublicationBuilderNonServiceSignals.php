...
final class CatalogPublicationBuilderNonServiceSignals
{
    ...
    /**
     * Услуги проектировщиков, визуализаторов, веб/UI-дизайнеров — не полевые бригады.
     * Консервативно: сначала отсекаем строителей/мастеров/сметчиков/ПТО, затем ищем явные роли проектирования.
     */
    private function matchesNonFieldConstructionService(string $original, string $lower): bool
    {
        ...
        if ($this->matchesExplicitDesignOfficeRole($lower)) {
            return true;
        }

        if ($this->matchesDesignProjectWithoutFieldExecution($lower)) {
            return true;
        }

        if ($this->matchesProjectDesignTeamOffer($lower)) {
            return true;
        }

        if ($this->matchesRevitBimOfficeDrafting($lower)) {
            return true;
        }
    ...
    /**
     * Явная роль: визуализатор, UX/UI, веб-дизайнер, BIM-менеджер, архитектор-проектировщик.
     */
    private function matchesExplicitDesignOfficeRole(string $lower): bool
    {
        $patterns = [
            '/\b(?:ux\s*\/\s*ui|ui\s*\/\s*ux|uxui)\b/u',
            '/\b(?:ux|ui)\s*[-\s]?(?:\/\s*)?(?:ux|ui)?\s*дизайн/u',
            '/\b(?:веб|web)[-\s]?дизайн/u',
            '/\b#(?:вебдизайнер|сайты|tilda|тильда)\b/u',
            '/\bдизайнер\s+сайт/u',
            ...
            '/\bbehance\.net\b/u',
            '/\b(?:тильд|tilda)\b/u',
            '/\bbim\s*[-\s]?менеджер\b/u',
            '/\bbim\s*[-\s]?разработчик\b/u',
            '/\b#bim[-\s]?разработчик\b/u',
            '/\b(?:инженер|специалист)[-\s]проектировщик\b/u',
            '/\b(?:занима(?:ю|ем)ся|специализиру(?:ю|ем)ся)\b.{0,56}\b(?:проектирован(?:ием|ие)|визуализ)/u',
            '/\b(?:архитектурно[-\s]строительн(?:ые|ой)\s+проект(?:ы|а|ов)?|проектирован(?:ие|ием)\s+(?:домов|коттедж|жилых))/u',
            '/\b(?:индивидуальное|эскизный)\s+проектирован/u',
            '/\b(?:проектиру(?:ю|ем)|разрабатыва(?:ю|ем)\s+проект)\b.{0,48}\b(?:дом|коттедж|инженерных\s+раздел)/u',
        ];
    ...
    /**
     * Дизайн-проект / визуализация как основная услуга без полевого исполнения.
     */
    private function matchesDesignProjectWithoutFieldExecution(string $lower): bool
    {
        if ($this->mentionsFieldExecution($lower)) {
            return false;
        }

        $patterns = [
            '/\b(?:разрабатыва(?:ю|ем)|выполня(?:ю|ем)|предлага(?:ю|ем)|оказыва(?:ю|ем))\b.{0,72}\b(?:дизайн[-\s]проект|3d\s*визуализ)/u',
            '/\b(?:дизайн[-\s]проект|3d\s*визуализ)\b.{0,72}\b(?:планировк|расстановк|рендер|визуализ)/u',
            '/\b(?:дизайнер|дизайн)\s+интерьер/u',
        ];
    ...
    /**
     * Команда проектировщиков / BIM-моделирование для экспертизы (МКА, IFC, разделы проекта).
     */
    private function matchesProjectDesignTeamOffer(string $lower): bool
    {
        if ($this->mentionsFieldExecution($lower)) {
            return false;
        }

        if (preg_match('/\b(?:команда|опытная\s+команда)\s+проектировщик/u', $lower) === 1) {
            return true;
        }

        if (preg_match('/\b(?:низко|высоко)полигональн/u', $lower) === 1
            && preg_match('/\b(?:мка|ifc|агр|проектиров|bim)\b/u', $lower) === 1) {
            return true;
        }

        if (preg_match('/\bifc\b.{0,24}\b(?:агр|agr)\b/u', $lower) === 1) {
            return true;
        }

        if ($this->countProjectDesignSectionAbbreviations($lower) >= 4
            && preg_match('/\b(?:раздел|проектиров|разработ(?:ка|аем)|команд[аы])\b/u', $lower) === 1) {
            return true;
        }
    ...
    /**
     * Revit/BIM-чертёж дизайн-проектов (не полевой монтаж).
     */
    private function matchesRevitBimOfficeDrafting(string $lower): bool
    {
        if ($this->mentionsFieldExecution($lower)) {
            return false;
        }

        if (preg_match('/\b#(?:чертежник|revit|ревит|bim|дизайн)\b/u', $lower) === 1
            && preg_match('/\b(?:#услуга|#помогу|предлага(?:ю|ем)|предоставля(?:ю|ем)|разрабатыва(?:ю|ем))\b/u', $lower) === 1) {
            return true;
        }

        if (preg_match('/\b(?:revit|ревит|bim)\b/u', $lower) === 1
            && preg_match('/\b(?:чертежник|рабоч(?:ая|ую)\s+документаци|дизайн[-\s]проект)\b/u', $lower) === 1) {
            return true;
        }

        if (preg_match_all('/\b(?:revit|ревит|bim|autocad|архикад|#чертежник|чертежник)\b/u', $lower) >= 2) {
            return true;
        }
    ...
    private function mentionsFieldExecution(string $lower): bool
    {
        return preg_match(
            '/\b(?:бригада|бригады|мастера[-\s]универсал|мастер(?:а|-)?(?:универсал|ов|а)|'.
            'ремонт|отделк|штукатур|маляр|плиточник|каменщик|электрик|электромонтаж|сантехник|'.
            'монтажник|разнорабоч|гипсокартон|кровел|фасад|сварк|фундамент|демонтаж|'.
            'кладк|газоблок|пеноблок|под\s+ключ|объект\w*\s+под\s+ключ|'.
            'изготовлени(?:е|я)\s+металл|металлоконструкци|'.
            'выполня(?:ем|ю|т)\b.{0,48}\b(?:ремонт|отделк|монтаж|квартир|объект\w*|фасад|кровл|штукатур|маляр|электр|сантех|демонтаж|кладк|под\s+ключ|бригад)|'.
            'дела(?:ем|ю|ет)\b.{0,48}\b(?:ремонт|отделк|монтаж|квартир|объект\w*|фасад|кровл|штукатур|маляр|электр|сантех|демонтаж|кладк)|'.
            'занима(?:ю|ем)ся\s+(?:ремонт|отделк|электромонтаж|сантех|монтаж|кровел|фасад|демонтаж)|'.
            '(?:ищем|ищу)\s+(?:объект|заказ|работу).{0,40}(?:под\s+ключ|отделк|ремонт)|'.
            'устройство\s+фасада|кровельн(?:ые|ая)\s+работ)\b/u',
            $lower
        ) === 1;
    }
    ...
    private function looksLikeFieldConstructionPerformer(string $lower): bool
    {
        ...
        $patterns = [
            '/\bбригада\b/u',
            '/\b(?:более|от)\s+\d{1,3}\s*\+?\s*(?:человек|рабоч|мастер)/u',
            '/\b\d{1,3}\s*\+?\s*человек/u',
            '/\b(?:в\s+)?поиск(?:е|ах)\s+(?:только\s+)?объект\w*\s+под\s+ключ/u',
            '/\b(?:готовы\s+)?рассмотреть\s+(?:ваши\s+)?предложения\s+по\s+ремонту/u',
            '/\b(?:сметчик|сметн(?:ое|ая|ую|ой))\b/u',
            '/\b(?:составлени(?:е|я)|разработ(?:ка|аем))\s+смет/u',
            '/\b(?:кс[-\s]?[23]|кс[-\s]?6)/u',
            '/\b(?:разработка|подготовка)\s+(?:ppr|пpr|пос|исполнительн)/u',
            '/\b(?:специалист|инженер)\s+пто\b/u',
            '/\b(?:обследован|строительн(?:ая|ой)\s+экспертиз|судебн(?:ая|ой)\s+стр)/u',
            '/\b(?:сантехническ|электромонтажн)\w*\s+работ/u',
            '/\b(?:фундамент|ленточн(?:ый|ого)|плитн(?:ый|ого)|свайн)/u',
            '/\b(?:ip|ип)\s*\+\s*частн(?:ая|ой)\s+практик/u',
            '/\b(?:сварк|отделк|отделочн|малярно[-\s]отделочн)/u',
            '/\b(?:комплексный\s+)?монтаж\s+(?:систем|инженер)/u',
            '/\b(?:профессиональн(?:ая|ой)\s+)?бригада\s+мастер/u',
            '/\b(?:ремонтно[-\s]строительн)/u',
            '/\b(?:демонтажник|грузчик|подсобн)/u',
        ];
