...
class AiResetSpecialistQueue extends Command
{
    protected $signature = 'app:ai_parse:reset-specialist-queue
                            ...
                            {--provider= : Провайдер ИИ (пустое значение — любой провайдер; для проектировщиков обычно yandexgtp4)}
    ...
    protected $description = 'Вернуть посты каналов типа «Проектировщик» в очередь ИИ (InQueue) для повторной обработки';
    ...
    public function handle(): int
    {
        ...
        if ($this->option('dry-run')) {
            ...
            $this->info('Условия: каналы Проектировщиков, период '.$from.' — '.$to.' по полю '.$dateField.', статусы: '.$statusLabel.', провайдер: '.($provider !== null && $provider !== '' ? $provider : 'любой'));
        ...
        foreach ($ids->chunk(500) as $chunk) {
            Specialist::query()->whereIn('api_channel_post_id', $chunk->all())->delete();
            ApiChannelPost::query()->whereIn('id', $chunk->all())->update($payload);
        }

        $this->info("Обновлено записей: {$count}. Запустите обработку: php artisan app:ai_parse:specialist");
