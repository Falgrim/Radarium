...
/**
 * Возвращает в очередь ИИ посты проектировщиков у авторов публичного каталога «Проектирование»,
 * у которых нет отображаемого Complete-сообщения (Complete + непустой текст поста).
 *
 * По умолчанию в очередь ставятся только посты со статусами Error, Empty, DontMatch (не Complete, не InQueue).
 */
class AiRequeueSpecialistMissingVisiblePosts extends Command
{
    ...
    protected $signature = 'app:ai_parse:requeue-specialist-missing-visible-posts
    ...
    protected $description = 'Поставить в очередь ИИ specialist-посты авторов каталога без видимого Complete-сообщения (dry-run / --apply + JSON-снимок)';
    ...
    public function handle(): int
    {
        ...
        $authorsQuery = $this->catalogAuthorsMissingVisibleSpecialistPostQuery();
        ...
        $this->line('Авторов в выборке (база каталога «Проектирование» без видимого Complete-текста): '.$authorCount);
        ...
        $snapshotPath = $backupDir.DIRECTORY_SEPARATOR.'requeue-specialist-missing-'.now()->format('Y-m-d_His').'.json';
        ...
        $this->info("Обновлено записей api_channel_posts: {$updated}. Запустите ИИ: php artisan app:ai_parse:specialist");

        return self::SUCCESS;
    }

    /**
     * Авторы как в публичном каталоге проектировщиков (без фильтров),
     * у кого нет активного specialist с постом Complete и непустым TRIM(post).
     */
    protected function catalogAuthorsMissingVisibleSpecialistPostQuery(): EloquentBuilder
    {
        return PublicSpecialistCatalogScope::publicCatalogAuthorsQuery()
    ...
    /**
     * Посты активных specialists этих авторов: канал Specialist, непустой текст, статус из списка, не InQueue.
     *
    ...
    protected function targetPostsQuery(Collection $authorIds, array $statuses): EloquentBuilder
    {
        ...
        return ApiChannelPost::query()
            ...
            ->join(Specialist::table(), 'specialists.api_channel_post_id', '=', 'api_channel_posts.id')
            ->where('api_channels.is_company', ApiDataTypeEnum::Specialist)
            ->whereIn('specialists.api_post_user_id', $authorIds)
            ->where('specialists.status', ApiPostAiStatusEnum::Active)
    ...
    protected function buildSnapshot(Collection $postIds, Collection $authorIds, array $statuses, ?int $limit): array
    {
        ...
        $specialists = Specialist::query()
            ...
            ->map(fn (Collection $group) => $group->map(fn (Specialist $s) => $s->getAttributes())->values()->all());

        return [
            'run_at' => now()->toIso8601String(),
            'command' => 'app:ai_parse:requeue-specialist-missing-visible-posts',
            'options' => [
                'apply' => true,
                'limit' => $limit,
            ...
            'specialists_by_post_id' => $specialists->toArray(),
        ];
    }
}
