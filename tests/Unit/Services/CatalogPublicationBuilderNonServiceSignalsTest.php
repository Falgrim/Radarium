...
TXT],
            ...
            'architect_projects' => ['Добрый день! Меня зовут Ирина, я выполняю архитектурно-строительные проекты домов'],
            'interior_visualizer' => ['Добрый день! Меня зовут Андрей, я #визуализатор #интерьеров'],
        ];
    }
    ...
    public static function fieldConstructionPerformerSamples(): array
    {
        return [
            ...
            'brigade_20_workers' => ['*Москва и Московская область* Бригада рабочих, 20+ человек. Мастера-универсалы. Качественно выполняем отделку'],
            'turnkey_brigade_search' => ['В ПОИСКАХ ТОЛЬКО ОБЪЕКТЫ ПОД КЛЮЧ. Москва и Московская область Бригада рабочих, 20+ человек'],
            'smetchik' => ['🔥ИНЖЕНЕР СМЕТЧИК Составление СМЕТ КС2 КС3 УСЛУГИ СМЕТЧИКА'],
            'smet_with_sections' => ['УСЛУГИ СМЕТЧИКА Составляем Сметы, КС-2, КС-3, АР, КЖ, КР, КМ, ВК, ОВ, СС, ЭС'],
            'facade_work' => ['Устройство фасада, ремонт фасада, отделка фасада, фасадные работы'],
            'repair_and_design_execution' => ['Выполняем дизайн-проекты и ремонт квартир. Опытные мастера. Гарантия.'],
            'pto_ppr' => ['САМОЗАНЯТЫЙ СПЕЦИАЛИСТ ПТО Разработка проектов производства работ ППР'],
            'building_survey' => ['Предлагаю услуги: Обследование зданий и сооружений Строительная экспертиза'],
            'metal_fabrication' => ['Изготовление металлоконструкций и изделий Сотрудничаем с подрядчиками'],
            'malyar_with_design_word' => ['Малярно отделочный мастер. От простой косметики до дизайн проекта'],
        ];
    }
