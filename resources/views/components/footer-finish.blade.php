<div class="container">
    <div class="row section-finish">
        <div class="col">
            <div class="finish-item">
                <div class="finish-text">
                    <h3 class="title-40semibold">Предложите<br>Свою Идею</h3>
                    <p>
                        <a href="https://t.me/radarium_tech" target="_blank"><img class="social" src="{{ asset('v2/img/icon-telegram.svg') }}" alt="Перейти в официальный канал поддержки"></a>
                        {{--<a href="#"><img class="social" src="{{ asset('v2/img/icon-whatsapp.svg') }}"></a>--}}
                    </p>
                </div>
                <div class="finish-img"><img src="{{ asset('v2/img/plastic-metall.png') }}"></div><a class="abs-link" href="https://t.me/radarium_tech" target="_blank" rel="noopener noreferrer"></a>
            </div>
        </div>
        <div class="col-12 col-lg-6">
            <div class="finish-item">
                <div class="finish-text">
                    <h1 class="title-40semibold">Техническая<br>Поддержка</h1>
                    <p>
                        <a href="https://t.me/radarium_tech" target="_blank"><img class="social" src="{{ asset('v2/img/icon-telegram.svg') }}" alt="Перейти в официальный канал поддержки"></a>
                        {{--<a href="#"><img class="social" src="{{ asset('v2/img/icon-whatsapp.svg') }}"></a>--}}
                    </p>
                </div>
                <div class="finish-img"><img src="{{ asset('v2/img/plastic-base.png') }}"></div><a class="abs-link" href="https://t.me/radarium_tech" target="_blank" rel="noopener noreferrer"></a>
            </div>
        </div>
    </div>
</div>
