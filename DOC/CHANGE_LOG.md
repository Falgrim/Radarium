...
- Побочный эффект: снятая или мягко удалённая карточка больше не держит автора вверху списка, а пост из чужой витрины (вакансия, строитель) не поднимает автора в каталоге проектировщиков.
- Каталог строителей получил пакетную подгрузку последнего сообщения вместо запросов на каждую строку; детальные страницы авторов перестали запрашивать один и тот же пост по три раза.
- «История сообщений» на странице автора по-прежнему показывает все обработанные сообщения (порядок от нового к старому), но сообщения из другого раздела каталога выделены бледным шрифтом (`.post-other-catalog`) — иначе непонятно, почему первая строка истории новее блока «Последнее сообщение».
- Миграция `2026_09_07_100000_add_catalog_last_message_indexes` — индексы `bl_user_status_idx`, `bl_post_id_idx`, `sp_post_id_idx` под новые подзапросы.
- Тесты: `tests/Unit/Support/CatalogLastMessageTest.php`.

---

## 2026-09-05

### AI / повторный прогон сообщений (консолидация)
- `AiReprocessService` — единая точка возврата сообщений в `InQueue` и запуска обработки; используется админкой и CLI.
- `ProcessPendingAiPosts` (queued job) — разбирает очередь пакетами по 100 до конца; останавливается без зацикливания, если прогресса нет (ИИ недоступен).
- `app:ai_parse:requeue` (`AiRequeuePosts`) — одна команда вместо двух зеркальных: **удалены** `app:ai_parse:reset-builder-queue` и `app:ai_parse:reset-specialist-queue`. Добавлена поддержка `company` и работа сразу по всем типам (`--type=all`).
- Админка «Источники сообщений»: кнопка «Обработать необработанные сообщения» (статусы Error/Empty → `InQueue` + запуск). Смена сервиса ИИ для всех источников и массовая установка статуса `InQueue` в «Сообщения/Посты» тоже запускают обработку через тот же сервис.
- Требуется запущенный `queue:work`.
